...
func CheckUser(user *model.User, email, password string) (bool, error) {
...
func GetUserId(email string) (uint64, error) {
	var user model.User
	if err := backend.ReadFromDBByKey(&user, "email", email, true); err == nil {
		return user.ID, nil
	} else if errors.Is(err, gorm.ErrRecordNotFound) {
...
func UserAddOrder(userId uint64, itemId uint64) error {
	order := model.Order{
		BuyerId: userId,
		ItemId:  itemId,
	...
	return backend.CreateRecord(&order)
}
...
func UserRemoveOrder(userId uint64, itemId uint64) (bool, error) {
	num, err := backend.DeleteFromDBByKeys(&model.Order{}, []string{"buyer_id", "item_id"},
		[]string{strconv.FormatUint(userId, 10), strconv.FormatUint(itemId, 10)})
	if err != nil {
		return false, err
	}
	if num == 0 {
	...
	if num > 1 {
		return false, util.ErrUnexpected("delete %d orders but expect only one", num)
	}
	return true, nil
}

// QuerySellerList returns the IDs of items in the list of seller whose ID is SELLER_ID
func QuerySellerList(sellerId uint64) ([]uint64, error) {
	var items []model.Item
	if err := backend.ReadFromDBByKey(&items, "seller_id", sellerId, false); err != nil {
		return nil, err
	}
	list := []uint64{}
